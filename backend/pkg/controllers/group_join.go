...
func ReplyRequestToJoinToGroup(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	groupID, err := parse.PayloadToString(message.Payload)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("Invalid payload for groupID: %s", message.Payload), err)
	}

	if webmodel.IsEmpty(groupID) {
		return nil, currConnection.WSBadRequest(app, message, "groupID is empty")
	}
	...
	_, _, err = currConnection.SendMessageToUser(app, group.CreatorID, webmodel.NewNotification, note)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("sending notification to user (id'%s') failed", group.CreatorID), err)
	}

	return groupID, nil
}

func ReplyInviteToGroup(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	userGroup, err := parse.PayloadToGroupUser(message.Payload)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("Invalid payload for userID: %s", message.Payload), err)
	}

	errmessage := userGroup.Validate()
	if errmessage != "" {
		return nil, currConnection.WSBadRequest(app, message, errmessage)
	}
	...
	_, _, err = currConnection.SendMessageToUser(app, userGroup.User, webmodel.NewNotification, note)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("sending notification to user (id'%s') failed", userGroup.User), err)
	}
	return userGroup.User, nil
}

func ReplyAcceptRequestToJoinGroup(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	...
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("set true member status for user (id '%s') in group (id '%s') failed", note.FromUserID.String, note.GroupID.String), err)
	}
	...
	_, _, err = currConnection.SendMessageToUser(app, note.FromUserID.String, webmodel.NewNotification, newNote)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("sending notification to user (id'%s') failed", note.FromUserID.String), err)
	}

	return note.GroupID.String, nil
}

func ReplyDeclineRequestToJoinGroup(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	note, err := handleJoinGroupNotification(app, currConnection, message, webmodel.NOTE_JOIN_GROUP_REQUEST)
	if err != nil {
		return nil, err // handleJoinGroupNotification sends errors to wsconnection
	}
	...
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("delete member user (id '%s') from group (id '%s') failed", note.FromUserID.String, note.GroupID.String), err)
	}
...
func ReplyAcceptInvitationToGroup(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	...
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("set true member status for user (id '%s') in group (id '%s') failed", note.UserID, note.GroupID.String), err)
	}
...
func ReplyDeclineInvitationToGroup(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	note, err := handleJoinGroupNotification(app, currConnection, message, webmodel.NOTE_INVITE_TO_GROUP)
	if err != nil {
		return nil, err // handleJoinGroupNotification sends errors to wsconnection
	}
	...
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("delete member user (id '%s') from group (id '%s') failed", note.UserID, note.GroupID.String), err)
	}
...
func handleJoinGroupNotification(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage, noteType string) (*models.Notification, error) {
	...
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("Invalid payload for notificationID: %s", message.Payload), err)
	}

	if notificationID < 1 {
		return nil, currConnection.WSBadRequest(app, message, "wrong notificationID")
	}
	...
	if note.Type != noteType || !note.GroupID.Valid || !note.FromUserID.Valid || note.Read != 0 {
		return nil, currConnection.WSError(app, fmt.Sprintf("invalide notification for '%s', got '%s'", noteType, note), err)
	}

	err = app.DBModel.MarkNotificationRead(notificationID)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("mark notification (id '%d') as read failed", notificationID), err)
	}
