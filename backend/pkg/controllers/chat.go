...
*/
func ReplyGetChatList(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	follows, err := app.DBModel.GetFollows(currConnection.Client.UserID)
	if err != nil {
		return nil, currConnection.WSError(app, "ReplyGetChatList: Failed to get search data from database", err)
	}
...
*/
func ReplySendMessageToChat(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	chatMessage, err := parse.PayloadToChatMessage(message.Payload)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("Invalid payload for a chat message: '%s'", message.Payload), err)
	}
	...
	if err != nil {
		return nil, currConnection.WSError(app, "get chat members from DB failed", err)
	}
	...
	jsonMessage, usersSentTo, err := currConnection.SendMessageToClientRoom(app, webmodel.InputChatMessage, chatMessage)
	if err != nil {
		return nil, currConnection.WSError(app, "sending message to client room failed", err)
	}
...
func sentMessageToUsersNotInChat(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage,
	...
	if len(memebersNotSentTo) > 0 {
		...
		usersSentTo = currConnection.SendBytesToUsers(app, memebersNotSentTo, jsonMessage)
...
func ReplyChatPortion(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	offset, err := parse.PayloadToInt(message.Payload)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("Invalid payload for the portion of messages '%s'", message.Payload), err)
	}
	...
	case PRIVATE_CHAT_ROOM:
		...
		if err != nil {
			return nil, currConnection.WSError(app, "get the next portion of chat messages from DB failed", err)
		}
