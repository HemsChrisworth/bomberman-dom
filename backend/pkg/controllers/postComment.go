...
func ReplyNewComment(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	comment, err := parse.PayloadToComment(message.Payload)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("Invalid payload for a new comment: %s", message.Payload), err)
	}
