...
func ReplyGetUserProfile(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	userID, err := parse.PayloadToString(message.Payload)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("Invalid userID '%s'", message.Payload), err)
	}
	...
	if user == nil {
		return nil, currConnection.WSBadRequest(app, message, fmt.Sprintf("cannot find a user with id '%s'", userID))
	}
...
func ReplySetProfileType(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	...
	if profileType < 0 || profileType >= webmodel.NUM_OF_PROFILE_TYPES {
		return nil, currConnection.WSBadRequest(app, message, "wrong profile type")
	}

	err = app.DBModel.SetUsersProfileType(currConnection.Client.UserID, profileType)
	if err != nil {
		return nil, currConnection.WSError(app, "get user from DB failed", err)
	}
