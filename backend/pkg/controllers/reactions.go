...
func ReplyReaction(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	reactionData, err := parse.PayloadToReaction(message.Payload)
	if err != nil {
		return nil, currConnection.WSError(app, fmt.Sprintf("Invalid payload in reaction request: %s", message.Payload), err)
	}
	...
	case models.COMMENT:
		react = liker.NewLikePost(currConnection.Client.UserID, currConnection.Client.UserName, reactionData)
	default:
		return nil, currConnection.WSError(app, fmt.Sprintf("unexpected message type in reaction request %s", reactionData.MessageType), errors.New("unexpected message type"))
	}

	if err = liker.SetLike(app.DBModel, react, reactionData.Reaction); err != nil {
		return nil, currConnection.WSError(app, "DB error during reaction handling", err)
	}

	// get the new number of likes/dislikes
	newReactions, err := react.GetLikesNumbers(app.DBModel)
	if err != nil {
		return nil, currConnection.WSError(app, "get new reactions failed", err)
	}
