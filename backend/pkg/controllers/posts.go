...
func ReplyDeletePost(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	...
	if err != nil {
		return nil, currConnection.WSError(app, "delete posts from DB failed", err)
	}
	err = deletePostImg(postID)
	if err != nil {
		if !os.IsNotExist(err) { // if post doesn't have an image, ignore error
			return nil, currConnection.WSError(app, "delete post image failed", err)
		}
	}
	return postID, nil
}

func ReplyPosts(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
	...
	if err != nil {
		return nil, currConnection.WSError(app, "getting posts from DB failed", err)
	}
...
func ReplyPostsInGroup(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
...
func ReplyUserPostsPortion(app *application.Application, currConnection *wsconnection.UsersConnection, message webmodel.WSMessage) (any, error) {
